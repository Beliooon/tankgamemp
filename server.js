...
app.get('/',function(req,res){
	res.sendFile(__dirname + '/html/server/index.html');
});

app.get('/html5game',function(req,res){	
	res.sendFile(__dirname + '/html/game/html5game/');
});

app.use('/game',express.static(__dirname + '/html/game'));
serv.listen(process.env.PORT|| 2000);

console.log("**Tank server is now running.");
...
var Player = function(id){
	var self = {
		...
		self.position = function(data){
				...
				self.drivingdirection2 = data.drivingdirection2;
		}
	...
	for(var p in Player.list){
	 var player = Player.list[p];
	  pack.push({
		...
		speed:player.speed,
		drivingdirection2:player.drivingdirection2,
